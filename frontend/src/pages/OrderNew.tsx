...
const OrderNew: React.FC = () => {
  ...
  const categories = ['Generico', 'Attrezzatura', 'Ricambi', 'Manutenzione', 'Altro'];
        ...
        <div className="max-w-4xl mx-auto">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      placeholder="Inserisci il titolo dell'acquisto"
                      className="text-sm font-bold text-emerald-600 bg-transparent focus:outline-none border-b-2 border-transparent hover:border-emerald-500 transition-all duration-200 w-full min-w-0"
                    />
                    <button
                      onClick={toggleStatus}
                      className={`px-5 py-1.5 rounded-full font-semibold text-sm text-white transition-all duration-300 min-w-[120px] text-center inline-flex items-center justify-center hover:scale-105 hover:shadow-md ${
                        formData.status === 'pending'
                          ? 'bg-orange-600 hover:bg-green-600'
                          : 'bg-green-600 hover:bg-orange-600'
                      }`}
                      title={`Clicca per cambiare in ${formData.status === 'pending' ? 'Effettuato' : 'Programmato'}`}
                      onMouseEnter={(e) => {
                        if (formData.status === 'pending') {
                          e.currentTarget.textContent = 'Effettuato';
                          e.currentTarget.classList.remove('bg-orange-600');
                          e.currentTarget.classList.add('bg-green-600', 'shadow-md');
                        } else {
                          e.currentTarget.textContent = 'Programmato';
                          e.currentTarget.classList.remove('bg-green-600');
                          e.currentTarget.classList.add('bg-orange-600', 'shadow-md');
                        }
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.textContent = formData.status === 'pending' ? 'Programmato' : 'Effettuato';
                        if (formData.status === 'pending') {
                    ...
                    >
                      {formData.status === 'pending' ? 'Programmato' : 'Effettuato'}
                    </button>
                  </div>
                  <div className="relative">
                    <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 text-2xl font-bold transition-colors duration-200 ${
                      formData.amount ? 'text-gray-800' : 'text-gray-400'
                    }`}>
                      €
                    </div>
                    <input
                      ...
                      placeholder="0.00"
                      className="text-3xl font-bold text-gray-800 bg-transparent focus:outline-none border-b-2 border-transparent hover:border-emerald-500 transition-all duration-200 w-full min-w-0 pl-6 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="category-input font-bold text-emerald-600 text-sm bg-transparent focus:outline-none w-full min-w-0"
          ...
          <div className="flex flex-col gap-2.5 mb-6">
            <button
              ...
              className="w-full py-3 px-4 rounded font-semibold text-sm transition-all duration-300 bg-emerald-500 hover:bg-emerald-600 text-white shadow"
            ...
            >
              {createMutation.isPending ? 'Creo...' : 'Aggiungi nuovo acquisto'}
            </button>
            <button
              onClick={() => navigate('/orders')}
